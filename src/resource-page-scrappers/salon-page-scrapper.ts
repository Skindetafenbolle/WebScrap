   ...
   setPage(page: Page) {
    ...
    async getSalonDescription(): Promise<string | null> {
        try {
            ...
            if (bntSelector) {
                const showMoreBtn = await this.page.$(bntSelector);
                if (showMoreBtn) {
                    await showMoreBtn.click();
                }
            }

            const descriptionElement = await this.page.$(this.resourceCfg.selectors.description);
            if (descriptionElement) {

                // @ts-ignore
                const description = await this.page.evaluate(el => el.textContent.trim(), descriptionElement);
                return description;
            } else {
